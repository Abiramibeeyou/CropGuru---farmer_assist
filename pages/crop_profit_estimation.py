...
import pandas as pd
from utils.crop_alternatives import CropAlternatives
from utils.Price_Estimation import comparative_profit_analysis
...
# Function to get price from API (as you already have)
def get_crop_prices(commodity):
    url = (
        "https://api.data.gov.in/resource/9ef84268-d588-465a-a308-a864a43d0070"
        "?format=json"
        "&api-key=579b464db66ec23bdd000001016f924a03f447ac49c092a8565a7933"
    ...
    try:
        ...
        if "records" in data and data["records"]:
            ...
            st.warning(f"No price data found for {commodity}")
            return None
    except Exception as e:
        st.error(f"Error fetching price for {commodity}: {e}")
        return None

# Revenue and profit estimation
def estimate_profit(yield_per_hectare, area, price_per_quintal, cultivation_cost_per_hectare):
    total_yield = yield_per_hectare * area
    total_income = total_yield * price_per_quintal
    total_cost = cultivation_cost_per_hectare * area
    profit = total_income - total_cost
    return profit

def main():
    st.title("Profit Estimation with User Inputs")

    recommended_crop = st.text_input("Enter Recommended Crop Name:")
    area = st.number_input("Enter Cultivation Area (hectares):", min_value=0.1, max_value=100.0, value=2.5)
    yield_per_hectare = st.number_input(f"Enter Yield for {recommended_crop.capitalize()} (quintals/ha):", min_value=0.0, value=40.0)
    cultivation_cost = st.number_input(f"Enter Cultivation Cost per hectare for {recommended_crop.capitalize()} (₹):", min_value=0.0, value=15000.0)

    if st.button("Estimate Profit"):
        price = get_crop_prices(recommended_crop)
        if price:
            profit = estimate_profit(yield_per_hectare, area, price, cultivation_cost)
            st.write(f"Price of the Crop: ₹{price} per quintal")
            st.success(f"Estimated Profit for {recommended_crop.capitalize()}: ₹{profit:,.2f}")
            
        else:
            st.error("Unable to fetch price, cannot estimate profit.")
