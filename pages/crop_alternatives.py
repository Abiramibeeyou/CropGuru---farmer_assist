...
<nav>
  <ul>
    <li><a href="/home";target="_top"><svg viewBox="0 0 24 24"><path d="M10 20v-6h4v6h5v-8h3L12 3 2 12h3v8z"/></svg>Home</a></li>
...
/* Style all buttons */
div.stButton > button {
    background-color: #1b3d17;  /* same as navbar bg */
    color: #f0f7f1;             /* light text */
    border: 2px solid #78be20;  /* bright green border */
    border-radius: 8px;
    padding: 8px 20px;
    font-weight: 600;
    font-size: 16px;
...
div.stButton > button:hover {
    background-color: #78be20;  /* bright green fill */
    color: #1b3d17;             /* dark text on green */
    border-color: #f0f7f1;      /* light border */
    cursor: pointer;
}

/* Focus effect (when clicked or tab-selected) */
div.stButton > button:focus {
    outline: none;
    box-shadow: 0 0 8px #78be20;
}
</style>
""", unsafe_allow_html=True)

def main():
    st.title("Crop Alternatives")

    st.write("Enter the crop name to see alternative crop suggestions based on similarity:")

    recommended_crop = st.text_input("Recommended Crop")
    ...
    if st.button("Get Alternatives"):
        if recommended_crop:
            ...
            if isinstance(alternatives, str):
                ...
                st.success(f"Top alternative crops to '{recommended_crop}':")
